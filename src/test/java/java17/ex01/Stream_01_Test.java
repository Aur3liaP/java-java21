...
public class Stream_01_Test {
	...
	public void test_stream_filter_adv() throws Exception {
		...
		// récupérer la liste de toutes les commandes qui contiennent au moins une
		// pizza Pépéroni
		List<Order> result = orders.stream().filter(o -> o.getPizzas().stream().anyMatch(p -> "Pépéroni".equals(p.getName()))).collect(Collectors.toList());
